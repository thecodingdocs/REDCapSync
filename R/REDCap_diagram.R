  ...
  if (static) {
    ...
    rendered_graph <- visNetwork::visNetwork(
      ...
      submain = DB$redcap$project_info$project_notes %>%
        paste0("<br>Code by Brandon Rose, M.D., M.P.H. at <a href='https://www.thecodingdocs.com/home'>TheCodingDocs.com</a> using <a href='https://github.com/thecodingdocs/REDCapSync'>REDCapSync</a> and <a href='https://github.com/datastorm-open/visNetwork'>VisNetwork</a>")
    ) %>%
    ...
    # }
    rendered_graph$x$options$groups <- rendered_graph$x$groups %>%
...
create_node_edge_REDCap <- function(
  ...
  if (DB$redcap$is_longitudinal) {
    ...
    edge_df <- edge_df %>% dplyr::bind_rows(
      data.frame(
        id = NA,
        from = events$arm_num %>% lapply(function(x) {
          node_df$id[which(node_df$group == "arm" & node_df$entity_name == x)]
        }) %>% unlist(),
        to = events$unique_event_name %>% lapply(function(x) {
          node_df$id[which(node_df$group == "event" & node_df$entity_name == x)]
        }) %>% unlist(),
        rel = NA, # "Belongs to",
        style = "filled",
        color = font.color,
      ...
      edge_df <- edge_df %>% dplyr::bind_rows(
        data.frame(
          ...
          to = event_mapping$form %>% lapply(function(x) {
            node_df$id[which(node_df$group == "form" & node_df$entity_name == x)]
          }) %>% unlist(),
          rel = NA, # "Belongs to",
          style = "filled",
          color = font.color,
