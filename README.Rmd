...
However, `{REDCapSync}` is the first "get-everything" REDCap R package that 
converts REDCap projects into a standardized, API-efficient, and 
project-agnostic [R6](https://r6.r-lib.org/index.html "R6 R package") object. 

## What is `{REDCapSync}`?

`{REDCapSync}` unleashes the full power of the REDCap API even for the basic R user. 
When a sync is performed, `{REDCapSync}` uses a cache of previous saves, a 
user-defined directory, and the REDCap log to only update data that changed 
since the last API call. Project objects can be used for the best that R has to 
