...
test_that("local_envvar seen by tests, exists, but empty at first", {
  ...
  # real cache path projects file path testing path projects.rds
  real_cache_path_projects2 <- file.path(testing_path2, "projects.rds")
  # expect false test directory exists testing path
  expect_directory_exists(testing_path2)
  # expect false test file exists real cache path projects
...
# cache_path (Internal)
test_that("cache_path works CONFIG overrides R_USER!", {
  user_cache <- cache_path()
  expect_true(nzchar(Sys.getenv("R_USER_CACHE_DIR")))
  expect_directory_exists(user_cache)
  expect_identical(user_cache, config$cache.dir())
  tempdir_test <- sanitize_path(withr::local_tempdir())
  withr::local_envvar(REDCAPSYNC_CONFIG_CACHE_DIR = tempdir_test)
  defined_cache <- cache_path()
  expect_identical(defined_cache, tempdir_test)
  expect_identical(defined_cache, config$cache.dir())
  withr::local_envvar(R_USER_CACHE_DIR = NA)
  expect_identical(Sys.getenv("R_USER_CACHE_DIR"), "")
  fake_other_cache <- hoardr::hoard()
  fake_other_cache$cache_path_set(full_path = cache_path_default())
  ...
  withr::local_envvar(REDCAPSYNC_CONFIG_CACHE_DIR = NA)
  expect_identical(Sys.getenv("REDCAPSYNC_CONFIG_CACHE_DIR"), "")
