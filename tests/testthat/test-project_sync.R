...
# test_that("sweep_dirs_for_cache handles missing project files", {
#   test_dir <- withr::local_tempdir() %>% sanitize_path()
#   fake_cache_location <- file.path(test_dir, "fake_cache")
#   local_mocked_bindings(
#     get_cache = function(...) {
...
#   )
#
#   # Setup initial project in cache only (no files on disk)
#   project <- mock_project()
#   project$dir_path <- set_dir(test_dir)
#   project_details <- extract_project_details(project)
#   add_project_details_to_cache(project_details)
...
#   expect_equal(nrow(projects_before), 1)
#
#   # Run sweep without project files on disk
#   expect_no_error(sweep_dirs_for_cache())
# })
#
# test_that("sweep_dirs_for_cache filters by specific project_names", {
#   test_dir <- withr::local_tempdir() %>% sanitize_path()
#   fake_cache_location <- file.path(test_dir, "fake_cache")
#   local_mocked_bindings(
#     get_cache = function(...) {
...
#
#   # Setup two projects
#   project1 <- mock_project()
#   project1$project_name <- "PROJECT_ONE"
#   project1$dir_path <- set_dir(test_dir)
#   project1_details <- extract_project_details(project1)
#   add_project_details_to_cache(project1_details)
#
#   project2 <- mock_project()
#   project2$project_name <- "PROJECT_TWO"
#   project2$dir_path <- set_dir(test_dir)
#   project2_details <- extract_project_details(project2)
#   add_project_details_to_cache(project2_details)
#
#   projects_before <- get_projects()
#   expect_equal(nrow(projects_before), 2)
#
#   # Sweep only PROJECT_ONE
#   expect_no_error(sweep_dirs_for_cache(project_names = c("PROJECT_ONE")))
#
#   # Both projects should still be in cache
#   projects_after <- get_projects()
#   expect_equal(nrow(projects_after), 2)
# })
