...
Precisely because tokens are sensitive, REDCapDB is designed to only reference the name of your token, such as "REDCapDB_TEST". If REDCapDB ever wants to use the token to make an API call to REDCap, it will check `Sys.getenv("REDCapDB_TEST")`. By default are token names start with "REDCapDB_", followed by the `short_name` you chose in `setup_DB()`.Below demonstrates how to set and check your tokens...

## Setting Your Token for One Session

```{r, eval=FALSE}
library(REDCapDB)
Sys.getenv("REDCapDB_TEST")
#>[1] ""
...
You can also set manually with base R. Unless you specifically set the token `Sys.getenv("REDCapDB_TEST")` will be blank.

```{r, eval=FALSE}
# Set your token manually
Sys.setenv(REDCapDB_TEST="a_FaKe_TOkEn") # again having this in a script is not advised but possible
...
# REDCapDB_TEST = "faKeTokeN"
# Save the file and Close
# Restart R Session (session tab)
# .rs.restartR() # this will also restart R session for you.
Sys.getenv("REDCapDB_TEST") # now should contain your token
```

The only reference to your token that is ever made or saved in REDCapDB is with its name. For example, any REDCap DB object will have the token name at `DB$token_name`. In the future, we may set our token in a different way. 
